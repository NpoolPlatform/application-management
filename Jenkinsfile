    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists application_management;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name application_management
            cd -
          done
        '''.stripIndent())
      }
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep application-management | grep latest | awk '{ print $3 }'`
